...
% keyboard;
  for j=1:n
    ...
    if(j<n)
      ...
      f = @(x) fun(x).*(1/h^2).*ddphi_1((x-a)/h).*(1/h^2).*ddphi_1((x-a)/h);
      M(2*j-1,2*j-1) = simpson(f,a,b,N);
      f = @(x) fun(x).*(1/h^2).*ddphi_1((x-a)/h).*(1/h).*ddphi_2((x-a)/h);
      M(2*j-1,2*j) = simpson(f,a,b,N);
      M(2*j,2*j-1) = M(2*j-1,2*j);

      f = @(x) fun(x).*(1/h^2).*ddphi_1((x-a)/h).*(1/h^2).*ddphi_3((x-a)/h);
      M(2*j-1,2*(j+1)-1) = simpson(f,a,b,N);
      f = @(x) fun(x).*(1/h^2).*ddphi_1((x-a)/h).*(1/h).*ddphi_4((x-a)/h);
      M(2*j-1,2*(j+1)) = simpson(f,a,b,N);
      ...
      f = @(x) fun(x).*(1/h).*ddphi_2((x-a)/h).*(1/h^2).*ddphi_3((x-a)/h);
    ...
    if(1<j)
      ...
      f = @(x) fun(x).*(1/h^2).*ddphi_3((x-a)/h).*(1/h^2).*ddphi_3((x-a)/h);
      M(2*j-1,2*j-1) = M(2*j-1,2*j-1) + simpson(f,a,b,N);
      f = @(x) fun(x).*(1/h^2).*ddphi_3((x-a)/h).*(1/h).*ddphi_4((x-a)/h);
      M(2*j-1,2*j) = M(2*j-1,2*j) + simpson(f,a,b,N);
      M(2*j,2*j-1) = M(2*j-1,2*j);

      f = @(x) fun(x).*(1/h^2).*ddphi_3((x-a)/h).*(1/h^2).*ddphi_1((x-a)/h);
      M(2*j-1,2*(j-1)-1) = simpson(f,a,b,N);
      f = @(x) fun(x).*(1/h^2).*ddphi_3((x-a)/h).*(1/h).*ddphi_2((x-a)/h);
      M(2*j-1,2*(j-1)) = simpson(f,a,b,N);
      ...
      M(2*j,2*(j-1)) = simpson(f,a,b,N);
      f = @(x) fun(x).*(1/h).*ddphi_4((x-a)/h).*(1/h^2).*ddphi_1((x-a)/h);
      M(2*j,2*(j-1)-1) = simpson(f,a,b,N);
    end
